// https://github.com/smartystreets/scanners/blob/master/csv/options.go
...
func Comma(comma rune) Option {
    return func(s *Scanner) { s.reader.Comma = comma }
}

func Comment(comment rune) Option {
    return func(s *Scanner) { s.reader.Comment = comment }
}
...
func LazyQuotes(lazy bool) Option {
    return func(s *Scanner) { s.reader.LazyQuotes = lazy }
}

func ReuseRecord(reuse bool) Option {
    return func(s *Scanner) { s.reader.ReuseRecord = reuse }
}

func TrimLeadingSpace(trim bool) Option {
    return func(s *Scanner) { s.reader.TrimLeadingSpace = trim }
}

func SkipHeaderRecord() Option {
    return SkipRecords(1)
}
